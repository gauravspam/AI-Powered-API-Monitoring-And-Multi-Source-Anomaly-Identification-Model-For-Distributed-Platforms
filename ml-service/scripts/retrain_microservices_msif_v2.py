...
feature_cols = [c for c in df.columns if c not in ['timestamp', 'label', 'experiment', 'fault_type_<lambda>', 'is_anomaly']]
...
x = LSTM(128, return_sequences=True, kernel_regularizer=tf.keras.regularizers.l2(0.0001))(input_layer)
...
callbacks = [
    ...
    ModelCheckpoint(os.path.join(MODEL_SAVE_PATH, "msif_lstm_model.keras"), save_best_only=True, monitor='val_auc', mode='max')
