...
"""LO2 Dataset Anomaly Detection Training"""
...
print(f"🚀 LO2 ANOMALY DETECTION TRAINING")
...
msif_model = keras.Sequential([

    keras.layers.Input(shape=(len(feature_cols),)),

    keras.layers.Reshape((1, len(feature_cols))),

    keras.layers.LSTM(128, activation='relu', return_sequences=True),  # Larger for LO2
...
msif_model.fit(
    ...
    callbacks=[
        ...
        keras.callbacks.ReduceLROnPlateau(monitor='val_auc', factor=0.5, patience=5, mode='max', min_lr=1e-6)
