...
def excluded_by_patterns(display_path: PurePosixPath, is_dir: bool, patterns: list[str]) -> bool:
    if not patterns:
        return False
...
def add_node(tree: Node, display_path: PurePosixPath, is_file: bool, note: str | None) -> None:
...
def cat_files(files: list[FileToCat], header: str, root: Path) -> int:
    for e in files:
        ...
        try:
            subprocess.run(["cat", "--", str(e.fp)], check=False)
...
def main() -> int:
    ...
    ap.add_argument("paths", nargs="+", help="Folders and/or files to process (relative or absolute).")
    ap.add_argument("--root", default=".", help="Root used to compute relative paths (usually repo root).")
    ap.add_argument("--max-bytes", type=int, default=1_000_000, help="Skip cat for files larger than this size (0 = no limit).")
    ...
    ap.add_argument("--header", default="==============", help="Header delimiter to print around relative path.")
    ap.add_argument("--tree", action="store_true", help="Print the combined tree before cat output.")
    ap.add_argument("--tree-only", action="store_true", help="Only print the tree and exit (implies --tree).")
    ap.add_argument("--ascii-tree", action="store_true", help="Use ASCII connectors (+--, `--) instead of Unicode.")
    args = ap.parse_args()
    ...
    def file_cat_exclusion_reason(fp: Path, disp: PurePosixPath, parent_blocked: bool) -> str | None:
        if parent_blocked:
    ...
    def handle_file(fp: Path, parent_blocked: bool) -> None:
        ...
        if args.max_bytes and st.st_size > args.max_bytes:
            add_node(tree, disp, is_file=True, note=f"(cat-skipped: too large {st.st_size} bytes)")
            return
