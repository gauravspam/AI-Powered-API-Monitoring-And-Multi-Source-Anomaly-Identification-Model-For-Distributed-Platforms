...
const prefetchData = () => {
    ...
    if (cachedDashboard.kpiCards?.length > 0) return Promise.resolve(cachedDashboard);
    
    initialFetchPromise = Promise.all([
            api.get("/overview").catch(() => ({ data: {} })),
            api
                .get("/anomalies/recent", { params: { limit: 20 } })
                .catch(() => ({ data: [] })),
            api
                .get("/metrics/traffic", { params: { limit: 50 } })
                .catch(() => ({ data: [] })),
        ])
            .then(([overviewRes, anomaliesRes, trafficRes]) => {
                const overview = overviewRes.data || {};
                const anomaliesData = (anomaliesRes.data || []).map((a) => ({
                    id: a.id,
                    serviceName: a.api_name || a.apiName || "",
                    endpoint: a.api_name || a.apiName || "",
                    environment: a.environment || "unknown",
                    severity: (a.severity || "").toLowerCase(),
                    score: a.final_anomaly_score ?? a.finalAnomalyScore ?? 0,
                ...
                const kpiData = [
                    ...
                    },
                    {
                        id: "anomalyRate",
                        label: "Anomaly Rate",
                        value: `${((overview.anomalyRate ?? 0) * 100).toFixed(2)}%`,
                        icon: WarningIcon,
                    },
                    {
                        id: "avgLatency",
                ...
                const envData = (overview.environmentSummary || []).map(
                    (env) => ({
                        name: env.environmentName ?? env.name ?? "unknown",
                        requestPerMin: env.requestsPerMinute ?? 0,
                        uptime: env.uptimePercent ?? 0,
...
export const Dashboard = () => {
    ...
    const [environmentSummary, setEnvironmentSummary] = useState(
        cachedDashboard.environmentSummary || [],
    );
    const [recentAnomalies, setRecentAnomalies] = useState(
        cachedDashboard.recentAnomalies || [],
    );
    const [trafficSeries, setTrafficSeries] = useState(
        cachedDashboard.trafficSeries || [],
    );
    ...
    const fetchDashboardData = useCallback(
        async (isPolling = false) => {
            try {
                ...
                if (
                    isPolling &&
                    Date.now() - lastFetchTime.current < THROTTLE_MS
                ) {
                    return;
                }

                const [overviewRes, anomaliesRes, trafficRes] = await Promise.all([
                    api.get("/overview").catch(() => ({ data: {} })),
                    api
                        .get("/anomalies/recent", { params: { limit: 20 } })
                        .catch(() => ({ data: [] })),
                ...
                const newEnvData = (overview.environmentSummary || []).map(
                    (env) => ({
                        name: env.environmentName ?? env.name ?? "unknown",
                        requestPerMin: env.requestsPerMinute ?? 0,
                        uptime: env.uptimePercent ?? 0,
                ...
                const newAnomaliesData = (anomaliesRes.data || []).map((a) => ({
                    id: a.id,
                    serviceName: a.api_name || a.apiName || "",
                    endpoint: a.api_name || a.apiName || "",
                    environment: a.environment || "unknown",
                ...
                const anomaliesChanged = shouldUpdate(
                    recentAnomalies,
                    newAnomaliesData,
                );
                const trafficChanged = shouldUpdate(
                    trafficSeries,
                    newTrafficData,
                );
    ...
    useEffect(() => {
        ...
        const loadData = async () => {
            ...
            if (
                prefetchFailed ||
                (cachedDashboard.kpiCards &&
                    cachedDashboard.kpiCards.length === 0)
            ) {
                // Reset flags
    ...
    const handleSeverityChange = useCallback(
        (e) => setSelectedSeverity(e.target.value),
        [],
    );
                                    ...
                                    >
                                        <Box sx={{ flex: 1 }}>
                                            ...
                                            <Typography
                                                ...
                                                color="text.secondary"
                                            >
                                                {env.requestPerMin.toLocaleString()}{" "}
                                                req/min
                                            </Typography>
                                        </Box>
                                        <Box
